...
if ($id == '' || $id == 0) {
    ...
    $query = $pdo->prepare("INSERT INTO usuarios SET nome = :nome, email = :email, 
        cpf = :cpf, senha = '$cpf', nivel = 'tesoureiros', id_pessoa = '$ult_id', foto = '$imagem'  ");
    ...
    if ($imagem == "sem-foto.jpg") {
        ...
        cpf = :cpf where id_pessoa = '$id' and nivel = 'tesoureiros'");
    } else {
        $query = $pdo->prepare("UPDATE usuarios SET nome = :nome, email = :email, 
        cpf = :cpf, foto = '$imagem' where id_pessoa = '$id' and nivel = 'tesoureiros'");
    }
    $query->bindValue(":nome", "$nome");
    $query->bindValue(":email", "$email");
    $query->bindValue(":cpf", "$cpf");
